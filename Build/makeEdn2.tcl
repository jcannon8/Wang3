# makeEdn2.tcl: Make CD3 epsilon-delta dimers with random CTs fused to truncated subunits
# with Nck bound to CD3 epsilon PRS.
# Output models, edn.*.[pdb, psf], have protein, membrane, and water.
# This is derived from makeCDed4.tcl.
# Nck bound used PDB 2JXB first frame.
...
# Use psfgen to build edn.$mod.[psf,pdb]
package require psfgen
# I should be able to use the Nck in the cd3edn model, but it is not working.
# Add the 2JXB Nck SH instead of the chain K within cd3edn.
set nckCD3 [mol new /home/jcannon/tcr2/cd3f/2jxb.pdb waitfor all]
# 2JXB has 20 NMR models, use first for binding. 
# CD3e PRS resid 6-12, Nck1 resid 31-86. 
set nckPRS [atomselect $nckCD3 "name CA and resid 6 to 12" frame 0]
set nckSH [atomselect $nckCD3 "resid 31 to 86" frame 0]
set nckAll [atomselect $nckCD3 all frame 0]
$nckAll set chain K
...
# Nck segid PROC, Glu31-Lys86. The chain D and E are backwards from makeCDed4.tcl.
...
set rPRS [atomselect $tcr2 "name CA and segid PROA and resid 655 to 661"]
...
##########
# A second cd3edn copy to change and save.
set tcr [mol new /home/jcannon/tcr16/charm-gui5/amber/cd3edn.psf]
...
##########
##########
# Moving the Nck chain K within tcr does not work right.
# Move Nck to bind to chain E PRS
# measure fit <moveable><reference>
set M [measure fit $rPRS $ePRS]
$chainK move $M
# Move 2JXB Nck to bind chain E PRS. This works!!
set M2 [measure fit $nckPRS $ePRS]
$nckAll move $M2  
set RMSD [measure rmsd $nckPRS $ePRS]
# ^^ The PRS RMSD is around 1.17 because of MD drift.
# Check if Nck collides with membrane
#set con [measure contacts 1.0 $chainK $mem]
...
set con [measure contacts 1.0 $nckSH $notPRS]
set flCon [llength [lindex $con 0]]
if {$flCon >0} {continue}
#######################################
...
#$chainK writepdb chainK1.pdb
# Use the aligned 2JXB Nck structure
$nckSH writepdb chainK2.pdb
...
#set conK [measure contacts 1.0 $chainK $water]
# Increase the waters to subtract by increasing distance 1.0 -> 1.5
# because nckSH does not have hydrogens.
set conK [measure contacts 1.5 $nckSH $water]
set wats [lindex $conK 1]
if {[llen $wats] >0} {
...
set addWat [atomselect $wat "beta 2 3 4 and not same residue as index $notWat2"]
# Water diagnostics
...
segment k {pdb chainK2.pdb}; coordpdb chainK2.pdb k
...
# Very important to add missing H atom coordinates for Nck!
guesscoord
writepsf edn.$mod.psf
writepdb edn.$mod.pdb
# Important to reset the psf for next model.
resetpsf
#######################################
# Check for loop penetration
set tcr3 [mol new edn.$mod.psf]
mol addfile edn.$mod.pdb waitfor all
